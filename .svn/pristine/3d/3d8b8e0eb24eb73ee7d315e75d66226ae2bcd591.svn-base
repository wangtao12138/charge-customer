...
public interface CustomerToMerchantService {
    ...
    PageData customerInfoList(CustomerInfoListDto customerInfoListDto);

    //根据chargingGuid查询使用记录
    DeviceUseDetailedVo queryChargingRecordByChargingGuid(String chargingGuid);


    public Integer setCustomerState(String customerGuid, Integer customerState) ;

    List<DayLineLossInfo> dayLineLoss( DayLineLossDto dayLineLossDto);

    List<UseDetailListInfo> querySchemeUseList(String projectGuid,String beginTime,String endTime);
}
