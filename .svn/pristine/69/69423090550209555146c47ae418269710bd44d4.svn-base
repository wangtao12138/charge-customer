...
//        iDeviceUseDetailedService.updateByPrimaryKeySelective(useDetailed);
     ...
     */
    @Override
    public Integer wxRefund(RefundParam refundParam) {
        ...
        log.info("微信退款返回WxRefundInfo={}",JSON.toJSONString(refundInfo));
     ...
     */
    @Override
    public Integer zfbRefund(RefundParam refundParam) {
        ...
        log.info("支付宝退款返回ZfbRefundInfo={}",JSON.toJSONString(refundInfo));
        Integer code = refundInfo.getCode();
        if (code == 0) {
            // 退款失败
            refund.setRefundState(CustomerConstant.RefundState.FAIL.getState());
